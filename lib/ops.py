...
def denselayer(inputs, output_size):
    ...
    denseLayer = tf.layers.Dense(
        output_size, activation=None, kernel_initializer=tf.contrib.layers.xavier_initializer()
    )
...
def upscale_four(inputs, scope="upscale_four"):  # mimic the tensorflow bilinear-upscaling for a fix ratio of 4
    with tf.variable_scope(scope):
        ...
        hi_res_bin = [
            [inputs, p_inputs[:, :-1, 1:, :]],  # top-left  # top-right
            [p_inputs[:, 1:, :-1, :], p_inputs[:, 1:, 1:, :]],  # bottom-left  # bottom-right
        ]
...
def bicubic_four(inputs, scope="bicubic_four"):
    ...
    with tf.variable_scope(scope):
        ...
        for hi in range(4):
            cur_wei = weights[hi]
            cur_data = (
                cur_wei[0] * hi_res_bin[0]
                + cur_wei[1] * hi_res_bin[1]
                + cur_wei[2] * hi_res_bin[2]
                + cur_wei[3] * hi_res_bin[3]
            )
...
def tf_data_gaussDownby4(HRdata, sigma=1.5):
    ...
    gau_list = np.float32(
        [[gau_k, gau_0, gau_0], [gau_0, gau_k, gau_0], [gau_0, gau_0, gau_k]]
    ...
    for var in var_list:
        ...
        if reader.has_tensor(tensor_name):
            ...
            if var.shape != npvariable.shape:
                raise ValueError(
                    "Wrong shape in for {} in ckpt,expected {}, got {}.".format(
                        var.name, str(var.shape), str(npvariable.shape)
                    )
                )
            ops.append(var.assign(npvariable))
        else:
            if print_level >= 1:
                print("variable not found in ckpt: " + var.name)
            if rest_zero:
                if print_level >= 1:
                    print("Assign Zero of " + str(var.shape))
...
def gif_summary(name, tensor, max_outputs, fps, collections=None, family=None):
    ...
    with summary_op_util.summary_scope(name, family, values=[tensor]) as (tag, scope):
        val = tf.py_func(py_gif_summary, [tag, tensor, max_outputs, fps], tf.string, stateful=False, name=scope)
