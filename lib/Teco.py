        ...
        with tf.compat.v1.name_scope("vgg_norm"):
            for key in output:
                if (deep_list is None) or (key in deep_list):
                    ...
                    if norm_flag:
                        orig_len = tf.sqrt(
                            tf.reduce_sum(input_tensor=tf.square(orig_deep_feature), axis=[3], keepdims=True) + 1e-12
                        )
...
def TecoGAN(r_inputs, r_targets, FLAGS, GAN_Flag=True):
    ...
    with tf.compat.v1.variable_scope("get_learning_rate_and_global_step"):
        ...
        learning_rate = tf.compat.v1.train.exponential_decay(
            FLAGS.learning_rate, global_step, FLAGS.decay_step, FLAGS.decay_rate, staircase=FLAGS.stair
        )
    ...
    with tf.device("/gpu:0"), tf.compat.v1.variable_scope("fnet"):
        ...
        fnet_input = tf.reshape(
            fnet_input, (FLAGS.batch_size * (inputimages - 1), FLAGS.crop_size, FLAGS.crop_size, 2 * output_channel)
        )
        # batch*(frame-1), FLAGS.crop_size, FLAGS.crop_size, output_channel
        ...
        gen_flow = tf.reshape(
            gen_flow, (FLAGS.batch_size, (inputimages - 1), FLAGS.crop_size * 4, FLAGS.crop_size * 4, 2)
        )
        ...
        input_frames = tf.reshape(
            Frame_t, (FLAGS.batch_size * (inputimages - 1), FLAGS.crop_size, FLAGS.crop_size, output_channel)
        )

    # tf.contrib.image.dense_image_warp, only in tf1.8 or larger, no GPU support
    s_input_warp = tf.contrib.image.dense_image_warp(
        tf.reshape(
            Frame_t_pre, (FLAGS.batch_size * (inputimages - 1), FLAGS.crop_size, FLAGS.crop_size, output_channel)
        ),
        gen_flow_lr,
    ...
    with tf.compat.v1.variable_scope("generator"):
        ...
        input0 = tf.concat(
            (
                r_inputs[:, 0, :, :, :],
                tf.zeros((FLAGS.batch_size, FLAGS.crop_size, FLAGS.crop_size, 3 * 4 * 4), dtype=tf.float32),
            ),
            axis=-1,
        )
        ...
        for frame_i in range(inputimages - 1):
            ...
            gen_pre_output_reshape = tf.reshape(
                gen_pre_output_warp, (FLAGS.batch_size, FLAGS.crop_size, 4, FLAGS.crop_size, 4, 3)
            )
            gen_pre_output_reshape = tf.transpose(a=gen_pre_output_reshape, perm=[0, 1, 3, 2, 4, 5])
            # batch,FLAGS.crop_size, FLAGS.crop_size, 4, 4, 3
            gen_pre_output_reshape = tf.reshape(
                gen_pre_output_reshape, (FLAGS.batch_size, FLAGS.crop_size, FLAGS.crop_size, 3 * 4 * 4)
            )
    ...
    s_gen_output = tf.reshape(
        gen_outputs, (FLAGS.batch_size * inputimages, FLAGS.crop_size * 4, FLAGS.crop_size * 4, 3)
    )
    ...
    if GAN_Flag:  # build the discriminator
        ...
        with tf.device("/gpu:0"), tf.compat.v1.name_scope("input_Tdiscriminator"):
            ...
            if not FLAGS.pingpang:  # backward motion has to be calculated
                fnet_input_back = tf.concat(
                    (r_inputs[:, 2:t_size:3, :, :, :], r_inputs[:, 1:t_size:3, :, :, :]), axis=-1
                )
                fnet_input_back = tf.reshape(
                    fnet_input_back, (t_batch, FLAGS.crop_size, FLAGS.crop_size, 2 * output_channel)
                )
                ...
                gen_flow_back = tf.reshape(
                    gen_flow_back, (FLAGS.batch_size, t_size // 3, FLAGS.crop_size * 4, FLAGS.crop_size * 4, 2)
                )

                T_inputs_VPre_batch = tf.identity(gen_flow[:, 0:t_size:3, :, :, :])  # forward motion reused,
                T_inputs_V_batch = tf.zeros_like(
                    T_inputs_VPre_batch
                ...
                T_inputs_VNxt_batch = tf.identity(
                    gen_flow[:, -2 : -1 - t_size : -3, :, :, :]
                )  # backward motion reused
            ...
            T_vel = tf.stop_gradient(
                T_vel
            )  # won't passing gradient to fnet from discriminator, details in TecoGAN supplemental paper

        if (
            FLAGS.crop_dt < 1.0
        ):  # crop out unstable part for temporal discriminator, details in TecoGAN supplemental paper
        ...
        with tf.compat.v1.name_scope("real_Tdiscriminator"):
            ...
            with tf.device("/gpu:0"), tf.compat.v1.variable_scope("tdiscriminator", reuse=False):
                real_warp = tf.reshape(
                    real_warp0, (t_batch, 3, FLAGS.crop_size * 4, FLAGS.crop_size * 4, 3)
                )  # [tb,T=3,h,w,ch=3 for RGB]
                real_warp = tf.transpose(a=real_warp, perm=[0, 2, 3, 4, 1])  # [tb,h,w,RGB,3T], 3T are t-1, t and t+1
                real_warp = tf.reshape(
                    real_warp, (t_batch, FLAGS.crop_size * 4, FLAGS.crop_size * 4, 9)
                )  # [tb,h,w,RRRGGGBBB] RRR: Red_t-1, Red_t, Red_t+1
                if FLAGS.crop_dt < 1.0:
                    real_warp = tf.image.crop_to_bounding_box(
                        real_warp, offset_dt, offset_dt, crop_size_dt, crop_size_dt
                    )

                if FLAGS.Dt_mergeDs:  # a spatio-temporal D
                    if FLAGS.crop_dt < 1.0:
                        real_warp = tf.pad(tensor=real_warp, paddings=paddings, mode="CONSTANT")
                    with tf.compat.v1.variable_scope(
                        "sdiscriminator", reuse=False
                    ):  # actually no more variable under this scope...
                        before_warp = tf.reshape(
                            t_targets, (t_batch, 3, FLAGS.crop_size * 4, FLAGS.crop_size * 4, 3)
                        )  # [tb,3=T,h,w,3=RGB]
                        before_warp = tf.transpose(a=before_warp, perm=[0, 2, 3, 4, 1])  # [b,h,w,3RGB,3T]
                        before_warp = tf.reshape(
                            before_warp, (t_batch, FLAGS.crop_size * 4, FLAGS.crop_size * 4, 3 * 3)
                        )  # [b,h,w,9=RRRGGGBBB]

                        t_input = tf.reshape(
                            r_inputs[:, :t_size, :, :, :], (t_batch, 3, FLAGS.crop_size, FLAGS.crop_size, -1)
                        )
                        t_input = tf.transpose(a=t_input, perm=[0, 2, 3, 4, 1])  # [tb,h//4,w//4,3RGB,3T]
                        t_input = tf.reshape(
                            t_input, (t_batch, FLAGS.crop_size, FLAGS.crop_size, -1)
                        )  # [tb,h//4,w//4,9=RRRGGGBBB]
                        input_hi = tf.image.resize(
                            t_input, (FLAGS.crop_size * 4, FLAGS.crop_size * 4)
                        )  # [tb,h,w,9=RRRGGGBBB]
                    ...
                    tdiscrim_real_output = discriminator_F(
                        real_warp, FLAGS=FLAGS
                    )  # [tb,h*FLAGS.crop_dt,w*FLAGS.crop_dt,RRRGGGBBB]
        ...
        with tf.compat.v1.name_scope("fake_Tdiscriminator"):
            ...
            with tf.device("/gpu:0"), tf.compat.v1.variable_scope("tdiscriminator", reuse=True):  # reuse weights
                ...
                if FLAGS.crop_dt < 1.0:
                    fake_warp = tf.image.crop_to_bounding_box(
                        fake_warp, offset_dt, offset_dt, crop_size_dt, crop_size_dt
                    )

                if FLAGS.Dt_mergeDs:  # a spatio-temporal D
                    if FLAGS.crop_dt < 1.0:
                        fake_warp = tf.pad(tensor=fake_warp, paddings=paddings, mode="CONSTANT")
                    with tf.compat.v1.variable_scope(
                        "sdiscriminator", reuse=True
                    ):  # actually no more variable under this scope...
                        before_warp = tf.reshape(
                            t_gen_output, (t_batch, 3, FLAGS.crop_size * 4, FLAGS.crop_size * 4, 3)
                        )
    ...
    with tf.compat.v1.variable_scope("generator_loss"):
        ...
        if FLAGS.vgg_scaling > 0.0:
            with tf.device("/gpu:0"), tf.compat.v1.variable_scope("vgg_layer_loss"):
                ...
                for layer_i in range(vgg_layer_n):
                    curvgg_diff = tf.reduce_sum(
                        input_tensor=gen_vgg[vgg_layer_labels[layer_i]] * target_vgg[vgg_layer_labels[layer_i]],
                        axis=[3],
                    )
        ...
        if GAN_Flag:  # spatio-temporal adversarial loss
            with tf.compat.v1.variable_scope("t_adversarial_loss"):
                ...
                dt_ratio = tf.minimum(
                    FLAGS.Dt_ratio_max, FLAGS.Dt_ratio_0 + FLAGS.Dt_ratio_add * tf.cast(global_step, tf.float32)
                )
    ...
    if GAN_Flag:  # Build the discriminator loss
        with tf.device("/gpu:0"), tf.compat.v1.variable_scope("t_discriminator_loss"):
            ...
            t_discrim_loss = tf.reduce_mean(
                input_tensor=-(t_discrim_fake_loss + t_discrim_real_loss)
            ...
            update_list += [
                tf.reduce_mean(input_tensor=tdiscrim_real_output),
                tf.reduce_mean(input_tensor=tdiscrim_fake_output),
            ]
        ...
        with tf.device("/gpu:0"), tf.compat.v1.variable_scope("tdicriminator_train"):
            ...
            tdiscrim_optimizer = tf.compat.v1.train.AdamOptimizer(
                tdis_learning_rate, beta1=FLAGS.beta, epsilon=FLAGS.adameps
            )
    ...
    with tf.compat.v1.variable_scope("generator_train"):
        ...
        if not GAN_Flag:
            ...
            with tf.control_dependencies(tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.UPDATE_OPS)):
                counter1 = tf.compat.v1.get_variable(
                    dtype=tf.int32,
                    shape=(),
                    name="gen_train_with_D_counter",
                    initializer=tf.compat.v1.zeros_initializer(),
                )
                counter2 = tf.compat.v1.get_variable(
                    dtype=tf.int32,
                    shape=(),
                ...
                # train D adaptively
                gen_train, fnet_train, gen_grads_and_vars, fnet_grads_and_vars = tf.cond(
                    pred=tf.less(tb, FLAGS.Dbalance), true_fn=train_gen_withD, false_fn=train_gen_withoutD
                )
    ...
    with tf.compat.v1.name_scope("image_summaries"):
        ...
        gif_sum = [
            gif_summary("LR", r_inputs, max_outputs=max_outputs, fps=3),
    ...
    Network = collections.namedtuple(
        "Network",
        "gen_output, train, learning_rate, update_list, "
