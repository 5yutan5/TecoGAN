    ...
    with tf.device("/cpu:0"):
        ...
        with tf.compat.v1.variable_scope("load_frame"):
            for dir_i in range(FLAGS.str_dir, FLAGS.end_dir + 1):
                ...
                if os.path.exists(inputDir):  # the following names are hard coded: col_high_
                    ...
                    image_list = [
                        os.path.join(inputDir, "col_high_%04d.png" % frame_i) for frame_i in range(FLAGS.max_frm + 1)
                    ]
                    image_set_lists.append(image_list)
            tensor_set_lists = tf.convert_to_tensor(value=image_set_lists, dtype=tf.string)
            input_slices = tf.compat.v1.train.slice_input_producer(
                [tensor_set_lists], shuffle=False, capacity=int(FLAGS.name_video_queue_capacity)
            )
            ...
            for frame_i in range(FLAGS.max_frm + 1):
                HR_data_i = tf.image.convert_image_dtype(
                    tf.image.decode_png(tf.io.read_file(input_slices[frame_i]), channels=3), dtype=tf.float32
                )
                HR_data += [HR_data_i]
        ...
        with tf.compat.v1.name_scope("sequence_data_preprocessing"):
            ...
            if FLAGS.random_crop and FLAGS.mode == "train":
                ...
                offset_w = tf.cast(
                    tf.floor(tf.random.uniform([], 0, tf.cast(HR_shape[-2], tf.float32) - tar_size)), dtype=tf.int32
                )
                offset_h = tf.cast(
                    tf.floor(tf.random.uniform([], 0, tf.cast(HR_shape[-3], tf.float32) - tar_size)), dtype=tf.int32
                )
            else:
            ...
            for fi in range(FLAGS.RNN_N):
                ...
                if FLAGS.random_crop and FLAGS.mode == "train":
                    HR_sequence_crop = tf.image.crop_to_bounding_box(
                        HR_sequence, offset_h, offset_w, tar_size, tar_size
                    )
                    ...
                    if FLAGS.movingFirstFrame:
                        ...
                        HR_sequence_crop_1 = tf.image.crop_to_bounding_box(
                            HR_data_0,
                            fix_off_h + lefttop_pos[fi][1],
                            fix_off_w + lefttop_pos[fi][0],
                            tar_size,
                            tar_size,
                        )

                        HR_sequence_crop = tf.compat.v1.where(
                            moving_decision < 0.7, HR_sequence_crop, HR_sequence_crop_1
                        )
        ...
        if len(target_images) != FLAGS.RNN_N:
            raise ValueError(
                "Length of target image sequence is incorrect,expected {}, got {}.".format(
                    FLAGS.RNN_N, len(target_images)
                )
            )

        print("Sequenced batches: {}, sequence length: {}".format(num_image_list_HR_t_cur, FLAGS.RNN_N))
        batch_list = tf.compat.v1.train.shuffle_batch(
            output_names + target_images,
            enqueue_many=True,
    ...
    with tf.device("/cpu:0"):
        ...
        for dir_i in range(FLAGS.str_dir, FLAGS.end_dir + 1):
            ...
            if os.path.exists(inputDir):  # the following names are hard coded
                ...
                for fi in range(FLAGS.RNN_N):
                    image_list_HR_r[fi] += [
                        os.path.join(inputDir, "col_high_%04d.png" % frame_i)
        ...
        with tf.compat.v1.variable_scope("load_frame"):
            ...
            output = tf.compat.v1.train.slice_input_producer(
                image_list_HR_r, shuffle=False, capacity=int(FLAGS.name_video_queue_capacity)
            )
            output_names = output
            ...
            if (
                FLAGS.movingFirstFrame and FLAGS.mode == "train"
            ):  # our data augmentation, moving first frame to mimic camera motion
                print("[Config] Use random crop")
                offset_xy = tf.cast(tf.floor(tf.random.uniform([FLAGS.RNN_N, 2], -3.5, 4.5)), dtype=tf.int32)
            ...
            for fi in range(FLAGS.RNN_N):
                HR_data = tf.image.convert_image_dtype(
                    tf.image.decode_png(tf.io.read_file(output[fi]), channels=3), dtype=tf.float32
                )
                if FLAGS.movingFirstFrame:
                    if fi == 0:
                    ...
                    HR_data_1 = tf.image.crop_to_bounding_box(
                        HR_data_0,
                        lefttop_pos[fi][1],
                        lefttop_pos[fi][0],
                    ...
                    HR_data = tf.cond(
                        pred=moving_decision < 0.7,
                        true_fn=lambda: tf.identity(HR_data),
                        false_fn=lambda: tf.identity(HR_data_1),
                    )
                    output_names[fi] = tf.cond(
                        pred=moving_decision < 0.7,
                        true_fn=lambda: tf.identity(output_names[fi]),
                        false_fn=lambda: tf.identity(output_names[0]),
                    )
        ...
        with tf.compat.v1.name_scope("data_preprocessing"):
            ...
            with tf.compat.v1.name_scope("random_crop"):
                ...
                if (FLAGS.random_crop is True) and FLAGS.mode == "train":
                    ...
                    offset_w = tf.cast(
                        tf.floor(tf.random.uniform([], 0, tf.cast(target_size[1], tf.float32) - tar_size)),
                        dtype=tf.int32,
                    )
                    offset_h = tf.cast(
                        tf.floor(tf.random.uniform([], 0, tf.cast(target_size[0], tf.float32) - tar_size)),
                        dtype=tf.int32,
                    )

                    for frame_t in range(FLAGS.RNN_N):
                        target_images[frame_t] = tf.image.crop_to_bounding_box(
                            target_images[frame_t], offset_h, offset_w, tar_size, tar_size
                        )
...
def frvsr_gpu_data_loader(FLAGS, useValData_ph):  # useValData_ph, tf bool placeholder, whether to use validationdata
    ...
    with tf.compat.v1.name_scope("load_frame_cpu"):
        ...
        with tf.compat.v1.name_scope("validation_data"):
            ...
            valFLAGS = copy_update_configuration(
                FLAGS,
                {
                    "str_dir": FLAGS.end_dir + 1,
                    "end_dir": FLAGS.end_dir_val,
                    "name_video_queue_capacity": val_capacity,
            ...
            for frame_t in range(FLAGS.RNN_N):
                ...
                target_images.append(
                    tf.image.crop_to_bounding_box(
                        curHR, k_w_border, k_w_border, FLAGS.crop_size * 4, FLAGS.crop_size * 4
                    )
                )
