...
from lib.dataloader import frvsr_gpu_data_loader, inference_data_loader
from lib.frvsr import fnet, generator_F
from lib.ops import *
...
Flags.DEFINE_boolean(
    "pre_trained_model",
    False,
    "If True, the weight of generator will be loaded as an initial point" "If False, continue the training",
)
...
Flags.DEFINE_integer(
    ...
    512,
    "The capacity of the filename queue (suggest large to ensure" "enough random shuffle.",
)
Flags.DEFINE_integer(
    "video_queue_capacity", 256, "The capacity of the video queue (suggest large to ensure" "enough random shuffle"
)
...
# The loss parameters
Flags.DEFINE_float(
    "vgg_scaling", -0.002, "The scaling factor for the VGG perceptual loss, disable with negative value"
)
...
def testWhileTrain(FLAGS, testno=0):
    ...
    cmd1 = [
        "python3",
        "main.py",  # never tested with python2...
        "--output_dir",
        desstr,
        "--summary_dir",
        desstr,
        ...
        "inference",
        "--num_resblock",
        "%d" % FLAGS.num_resblock,
        "--checkpoint",
        os.path.join(FLAGS.output_dir, "model-%d" % testno),
        "--cudaID",
        FLAGS.cudaID,
    ]
    ...
    cmd1 += [
        "--input_dir_LR",
        "./LR/calendar/",  # update the testing sequence
        "--output_pre",
        "",  # saving in train folder directly
        "--output_name",
        "%09d" % testno,  # name
        "--input_dir_len",
        "10",
...
if FLAGS.mode == "inference":
    ...
    with tf.compat.v1.variable_scope("generator"):
        ...
        with tf.compat.v1.control_dependencies([tf.assign(pre_inputs, inputs_raw)]):
        ...
        try:
            for step in range(max_iter):
                ...
                if (run_step % FLAGS.display_freq) == 0:
                    ...
                    print(
                        "progress  epoch %d  step %d  image/sec %0.1fx%02d  remaining %dh%dm"
                        % (train_epoch, train_step, rate, frame_len, remaining // 3600, (remaining % 3600) // 60)
                    )
