...
for folder_i in range(folder_n):
    ...
    for i in range(cutfr, image_no - cutfr):
        ...
        if (target_img.shape[0] < output_img.shape[0]) or (
            target_img.shape[1] < output_img.shape[1]
        ):  # target is not dividable by 4
    ...
    for cur_num_data in keys:
        ...
        print("%s, max %02.4f, min %02.4f, avg %02.4f" % (num_data, cur_list.max(), cur_list.min(), num_data_mean))
...
for num_data in keys:
    ...
    print(
        "%s, total frame %d, total avg %02.4f, folder avg %02.4f"
